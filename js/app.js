const app = {
            ...
            async init() {
                // Show local JSON immediately so the page never looks empty
                try {
                ...
                // Real-time cloud listener - once products exist in Firestore,
                // EVERY visitor's page updates instantly, automatically.
                this.listenToCloud();
            },
            ...
            listenToCloud() {
                ...
                db.collection('products').orderBy('createdAt', 'desc').onSnapshot((snapshot) => {
                    if (snapshot.empty) return; // nothing imported yet, keep showing local JSON
                    const cloudProducts = [];
            ...
            async deleteProduct(index) {
                ...
                if (p.id && typeof db !== 'undefined') {
                    try {
                        ...
                        this.showToast('Product deleted for everyone');
                    } catch (err) {
            ...
            async importToCloud() {
                ...
                if (!confirm(`Import ${this.products.length} current products to the cloud? Do this only once.`)) return;
                this.showToast('Importing... please wait');
                try {
                    ...
                    await batch.commit();
                    this.showToast('Imported! Live for everyone now.');
                } catch (err) {
                    console.error(err);
